 var dataset;
    ...
    //  	  function move (percentage) {
        ...
        function init() {
            ...
            window.count = 0;
					  ...
					  //alert(video_hour + " " + video_minute + " " + video_secound);
            var st = new Date("2015-03-25T00:00:00Z");
					 ...
					 //  document.getElementById('zoomIn').onclick    = function () { timeline.zoomIn( 0.2); timeline2.zoomIn( 0.2);};
					  ...
					  timeline.on('click', function(properties){
							  var seconds = properties.time.getHours()*3600 + properties.time.getMinutes()*60 + properties.time.getSeconds();
        ...
        function pauseAll() {
            ...
            window.count = Math.round(video1.currentTime * 4.89);
        ...
        function Distance() {          
            ...
            for (var i = 0; i < l; i++) {
                ...
                showtext += "Distance To Screen: " + ((419 - window.dataset[window.count].skeletons[i].head.y)/100 * 0.6).toFixed(2) + " m<br>";
                ...
                button.on("click", function() {
                    ...
                    var textLabel = new createjs.Text("Select an entity to create a distance query \nbetween person " + (pNo + 1) + " and it", " 14px Courier", "#00000");
                    textLabel.textAlign = "left";
                    textLabel.x = 100;
                    textLabel.y = 0;
                    stage.addChild(textLabel);

                    var textLabel2 = new createjs.Text("Drag \"Smaller\" or \"Larger\" button to zoom in or zoom out \nthe green circle to get a specific distance", " 14px Courier", "#00000");
                    ...
                    backgroundOS.graphics.beginFill("#00f000").drawRoundRect(0, 0, 120, 30, 5);
                    ...
                    backgroundOL.graphics.beginFill("#00f000").drawRoundRect(0, 0, 120, 30, 5);
   ...
   line3.graphics.moveTo(x,y).setStrokeStyle(2).beginStroke("#5CB85B").lineTo(40,110);
                    ...
                    sDragger.on("pressmove", function(evt) {
                        ...
                        sLabel.text=(newDistance * 0.6).toFixed(2);
                    ...
                    sDragger.on("pressup", function(evt) {
                        ...
                        var sRect = new createjs.Shape();
                        sRect.graphics.beginFill("#00f000").drawRoundRect(-(recWidth / 2), originalRadius - (recHeight / 2), recWidth, recHeight, 5);
                        var sLabel = new createjs.Text((originalRadius * 0.6) .toFixed(2), " 12px Courier", "#00000");
                        sLabel.textAlign = "center";
                        sLabel.x += recWidth / 2 - 25;
                        sLabel.y += originalRadius - (recHeight / 2) + 6;
                    ...
                    buttonOS.on("click", function() {
                        ...
                        textLabel2.alpha = 0;
                        textBuffer = h + " . Distance between person " + (pNo + 1) + " and " + enetityName + " is smaller than " + (originalRadius / 100).toFixed(2) + " m.";
												
												var d = {"obj1": "person " + (pNo + 1), "obj2": enetityName, "relation": "s", "distancebetween": (newDistance / 100).toFixed(2)};
												window.searchData.distance.push(d);
                        makeQuery(textBuffer);
                     ...
                     buttonOL.on("click", function() {
                        ...
                        sDragger.alpha = buttonOS.alpha = buttonOL.alpha= 0;
                        textLabel2.alpha = 0;
                        textBuffer = h + " . Distance between person " + (pNo + 1) + " and " + enetityName + " is larger than " + (newDistance / 100).toFixed(2) + " m.";
												 
												var d = {"obj1": "person " + (pNo + 1), "obj2": enetityName, "relation": "l", "distancebetween": (newDistance / 100).toFixed(2)};
												window.searchData.distance.push(d);
                        makeQuery(textBuffer);
                        // makeQuery is the function to add both text and button dynamically to the queryView
                        // It takes textBuffer, which is got before, as the only parameter. 
        ...
        function Location() {
            ...
            var textLabel = new createjs.Text("Click Draw button to select locations by drawing. \nClick Finish button to finish and create a query", " 14px Courier", "#00000");
                textLabel.textAlign = "left";
                textLabel.x = 4;
                textLabel.y = 0;
                stage.addChild(textLabel);
            ...
            buttonO.on("click", function() {
                ...
                window.searchData.location.push(locationData);
                

               
                // Using h as a counter, h is a global variable, and will be added 1 whenever it is called. So the printed out query would not be messed up with each other. 

                textBuffer = h + " Someone is on one of these locations.";
                makeQuery(textBuffer);
            })
            ...
            for (var i = 0; i < l; i++) {
                ...
                button.on("click", function() {
                    ...
                    pointC.on("click", function() {
                        ...
                        textBuffer = h + " : Activity of person " + (pNo + 1) + " is pointing";
                    ...
                    standC.on("click", function() {
                        ...
                        closeC.alpha = 1;
                        h = h + 1;
                        textBuffer = h + " : Activity of person " + (pNo + 1) + " is standing";
                    ...
                    tabletC.on("click", function() {
                        ...
                        h = h + 1;
                        textBuffer = h + " . Activity of person " + (pNo + 1) + " is using tablet";
                    ...
                    paperC.on("click", function() {
                        ...
                        closeC.alpha = 1;
                        h = h + 1;
                        textBuffer = h + " . Activity of person " + (pNo + 1) + " is using  paper";
                    ...
                    phoneC.on("click", function() {
                        ...
                        textBuffer = h + " . Activity of person " + (pNo + 1) + " is using  phone";
        ...
        function showCommenttimeline(cmntT, cmnTcnt){
            var dbase = new Date("2015-03-25T00:00:00Z");
             ...
             if((dbase.getSeconds()+cmntT)/60 > 1){
                                    dbase.setSeconds(obase.getSeconds()+(dbase.getSeconds()+cmntT)%60);
		  ...
		  function showtimeline(){
				  ...
				  for(var s=0; s<window.dataset.length; s++){
						  ...
						  for(var i=0; i<window.searchData.distance.length; i++){
								  ...
								  if(item.obj2 == "large screen"){
										  ...
										  if(pid > l){
												  ...
												  var d = (Math.sqrt(a*a+b*b)/100).toFixed(2);
                                                  //smaller than
												  if(item.relation == "s"){
														  if(d < item.distancebetween + 20){
															}
														  else{
																  disrange.push(s);
																  break;
						  ...
						  for(var i in window.searchData.orientation){
								  ...
								  if(pid > l){
										  ...
										  if(window.dataset[tscount].skeletons[pid-1].head.orientation < parseFloat(item.orient)+30.00 && window.dataset[tscount].skeletons[pid-1].head.orientation > parseFloat(item.orient)-30.00){
											}
										  else{
												  orirange.push(s);
												  break;
						  ...
						  for(var i=0; i<window.searchData.motion.length; i++){
                            ...
                            for(var i=0; i<window.searchData.location.length; i++){
                                var item = window.searchData.location[i];
                                // console.log(s,item);
                                var check =false;
                                ...
                                for(var j=0; j<item.length; j++){
                                     ...
                                     for(var p=0; p<l; p++){
                                        // console.log(p,s, "headx",window.dataset[tscount].skeletons[p].head.x,"x",lox);
                                        // console.log(p,s,"heady",window.dataset[tscount].skeletons[p].head.y,"y",loy);
				  ...
				  if(disrange.length == 1){
						  ...
						  if((dbase.getSeconds()+tstartd)/60 > 1){
									dbase.setSeconds(obase.getSeconds()+(dbase.getSeconds()+tstartd)%60);
